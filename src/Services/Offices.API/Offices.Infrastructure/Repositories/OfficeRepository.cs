...
public class OfficeRepository : IOfficeRepository
{
	...
	public async Task<ICollection<Office>> GetAllAsync(CancellationToken cancellationToken = default) =>
		(await _context.Offices.FindAsync(Builders<Office>.Filter.Empty)).ToList();
		
	public async Task<Office> GetByIdAsync(Guid id, CancellationToken cancellationToken = default) =>
		(await _context.Offices.FindAsync(Builders<Office>.Filter.Eq(x => x.Id, id))).FirstOrDefault();
}
