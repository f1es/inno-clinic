...
public class OfficeRepository : IOfficeRepository
{
	...
	public async Task<IEnumerable<Office>> GetAllAsync() => await (await _context.Offices.FindAsync(Builders<Office>.Filter.Empty)).ToListAsync();
	public async Task<Office> GetByIdAsync(Guid id) => await _context.Offices.Find(Builders<Office>.Filter.Eq(x => x.Id, id)).FirstOrDefaultAsync();
}
